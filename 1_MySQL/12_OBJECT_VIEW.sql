...
timestampdiff(year,hire_date, now()) 근속년수
FROM employee
JOIN department ON(dept_code = dept_id)
...
UPDATE vw_emp_sal
SET 급여 = 9000000
WHERE 사번 = 200; -- 산술 연산과 무관한 컬럼은 데이터 변경 가능!
...
SELECT dept_code,sum(salary) 합계 ,format(avg(salary),0) 평균
FROM employee
JOIN department ON (dept_code = dept_id)
GROUP BY dept_code;
...
-- employee 테이블로 job_code만 중복 없이 조회한 SELECT문을 vw_dt_job 뷰 정의
