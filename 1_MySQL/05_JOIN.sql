...
SELECT dept_code,emp_name,bonus,dept_title
FROM employee
JOIN department ON (dept_code = dept_id)
WHERE dept_title = '인사관리부';

-- 2. 전체 부서의 부서코드, 부서명, 지역코드, 지역명 조회(department, location) -- location id, local_code
...
SELECT job_code,emp_name,bonus,dept_title
FROM employee
JOIN department ON (dept_code = dept_id)
...
SELECT * FROM employee; -- emp_id(사번) emp_name(직원명) salary(급여)       dept_code, job_code
SELECT * FROM department; -- dept_title                                   dept_id,            location_id        
SELECT * FROM job;        -- job_name                                                job_code
SELECT * FROM location;   --  local_name                          national_code                           local_code                                             
...
SELECT dept_title, format(sum(salary),0) "급여 합"
FROM employee
     JOIN department ON(dept_code = dept_id)
group by dept_title
...
SELECT emp_id, emp_name, job_name, sal_level,
       if(sal_level IN('S1', 'S2'), '고급', 
        ...
        CASE WHEN sal_level IN('S3','S4') THEN '중급'
        ELSE '초급'
	END "구분2"
FROM employee
     JOIN job USING(job_code)
...
-- 11. 이름에 '형'자가 들어 있는 직원들의 사번, 직원명, 직급명 조회
