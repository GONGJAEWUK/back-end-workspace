...
-- (1) 부서의 합이 가장 큰 값
-- (2) 테이블을 만들어서 max 사용
-- (3) 만든 테이블을 HAVING 절을 통해 서브쿼리 만듬
...
*/
-- /각 부서별 최고 급여/를 받는 직원의 이름,직급코드,부서코드, 급여 조회
-- 1) 각 부서별 최고 급여
SELECT max(salary)
FROM employee
...
-- 하이유 사원과 같은 부서코드, 같은 직급 코드에 해당 하는 사원들의 사원명, 부서코드, 직급코드 조회
...
SELECT emp_id,emp_name,ifnull(dept_code,'부서없음') 부서,salary
FROM employee
