...
/* 
   ...
   CHAR-LENGTH : 해당 문자열값의 글자 수 반환 
   
*/
SELECT 

       length('데이터베이스'), char_length('데이터베이스'), length('database'), char_length('database');
...
SELECT trim(LEADING 'Z' FROM 'ZZZKHZZZ');-- KHZZZ <--- 문자제거는 LEADING
...
SELECT emp_name,email,replace(email,'kh.or.kr','gmail.com') "변경 후"
FROM employee;

-- 2. 사원명과 주민번호(000000-0******)으로 조회

SELECT emp_name,emp_no, replace(emp_no, substr(emp_no,9,7),'******')
FROM employee;
...
SELECT emp_name,hire_date,timestampdiff(day, hire_date , now()) "근무 일 수", timestampdiff(month,hire_date,now()) "근무 월 수",
...
-- 직원명, 입사일(2024년 06월 19일 14시 05분 30초)
...
SELECT emp_name,coalesce(bonus,0) 보너스
FROM employee;

-- 전 사원의 직원명, 보너스, 보너스 포함 연봉(급여 + 급여 * 보너스) * 12 조회

SELECT emp_name,IFNULL(bonus,0) "보너스" ,(salary+salary*IFNULL(bonus,0))* 12 "보너스 포함 연봉"
FROM employee;
...
SELECT nullif('123','123'), nullif('123','452'); -- null, 123
...
SELECT dept_code,emp_name,emp_no,if(substr(emp_no,-7,1) = 1,'남','여')
FROM employee;
...
SELECT emp_name,job_code,salary,format(if(job_code = 'J7',salary * 1.1,if(job_code = 'J6',salary * 1.15,
if(job_code = 'J5',salary * 1.2,salary * 1.05))),0) "인상된 급여"
FROM employee
ORDER BY job_code,salary desc;
...
format(CASE WHEN job_code = 'J7' Then salary * 1.1
                                     ...
                                     WHEN job_code = 'J6' Then salary * 1.2
                                     else salary * 1.05
                                     end ,0)"인상된 급여"
FROM employee
ORDER BY job_code,salary desc;
...
/*
   ...
   * : 조회 결과에 해당하는 모든 행 개수 반환
   ...
   distict 컬럼 : 해당 컬럼값의 중복을 제거한 행 개수 반환 
*/
-- 전체 사원 수 조회
SELECT count(*)
FROM employee;
