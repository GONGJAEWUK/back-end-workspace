...
ALTER TABLE dept_copy DROP COLUMN cname; -- 삭제 불가능!
...
-- 삭제 하고 싶다면 자식 테이블 먼저 삭제 후 부모 테이블 삭제 가능!
DROP TABLE member;
